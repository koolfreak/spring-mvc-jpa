...
function PopWindow(el) {
	...
	if (this.clipTextMinimumWidth >= this.minimumWidth)
		this.titleBarText.style.width = (winCtrl.minimizedTextWidth + initWd - this.minimumWidth)
				+ "px";
...
function winResizeDragGo(event) {
	...
	if (east || west) {
		...
		if (browser.isIE)
			winCtrl.active.clientArea.style.width = (w - winCtrl.active.widthDiff)
					+ "px";
	}
	if (north || south)
	...
	if (east || west) {
		if (w < winCtrl.active.clipTextMinimumWidth)
			winCtrl.active.titleBarText.style.width = (winCtrl.minimizedTextWidth
					+ w - winCtrl.active.minimumWidth)
					+ "px";
		else
...
function openNewPopWindow(bookingCode) {
	
	if(winList[bookingCode])
	{
		winList[bookingCode].open();
	}
	else
	{
		CreatePopWindow.createWin(bookingCode);
		var newObject = document.getElementById(bookingCode);
		winList[bookingCode] = new PopWindow(newObject);
		WayBillPopUpEvent.populatePopUp(bookingCode);
		winList[bookingCode].open();
	}
}
